...
sys_exofork(void)
{
	...
	parent = curenv;
	if((res = env_alloc(&child, parent->env_id)) < 0)
	{
		return res;
	}
...
sys_page_alloc(envid_t envid, void *va, int perm)
{
	...
	if ((uint32_t)va >= UTOP || (uint32_t)va & (PGSIZE - 1))
	{
		return -E_INVAL;
	}
	...
	if ((res = page_insert(e->env_pgdir, pp, va, perm)) < 0)
	{
		page_free(pp);
		return res;
	}
	     ...
	     envid_t dstenvid, void *dstva, int perm)
{
	...
	if ((uint32_t)srcva >= UTOP || (uint32_t)srcva & (PGSIZE - 1) || (uint32_t)dstva >= UTOP || (uint32_t)dstva & (PGSIZE - 1))
	{
		return -E_INVAL;
	}
	if (!(perm & PTE_U) || !(perm & PTE_P) || (perm & ~PTE_SYSCALL))
	...
	if (((res = envid2env(srcenvid, &srce, 1)) < 0) || ((res = envid2env(dstenvid, &dste, 1)) < 0))
	{
		return res;
	}
	...
	if ((res = page_insert(dste->env_pgdir, pp, dstva, perm)) < 0)
	{
		return res;
	}
	return 0;
